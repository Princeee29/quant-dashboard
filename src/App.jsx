...
const StatusBadge = ({ status }) => {
  ...
  return (
    <div className={`flex items-center justify-center gap-1 px-2 py-1 rounded text-[10px] font-bold border w-[80px] uppercase tracking-wider shadow-sm transition-all duration-300 ${
      isWin 
        ? 'border-green-500/30 text-green-400 bg-green-500/5 shadow-[0_0_10px_rgba(34,197,94,0.1)]' 
        : 'border-red-500/30 text-red-400 bg-red-500/5 shadow-[0_0_10px_rgba(239,68,68,0.1)]'
    }`}>
      {isWin ? <ArrowUpRight size={10} strokeWidth={3} /> : <ArrowDownRight size={10} strokeWidth={3} />}
      {status}
    </div>
  );
};
  ...
  <div className="bg-[#0A0A0A] border border-[#1C1C1C] rounded-2xl p-5 flex flex-col justify-between hover:border-gray-700 hover:bg-[#0f0f0f] transition-all duration-300 group h-[115px] relative overflow-hidden">
    ...
    <div className="flex items-center justify-between text-gray-500 text-[10px] font-bold tracking-wider uppercase z-10">
      <div className="flex items-center gap-2">
        {Icon && <Icon size={14} className="text-gray-600 group-hover:text-green-500 transition-colors" />}
        {label}
      </div>
...
const AdvancedChart = ({ data }) => {
  ...
  reversedData.forEach(t => {
      ...
      dailyDataMap[dateKey] += parseFloat(t.pnl);
  });
  const dailyData = Object.keys(dailyDataMap).map(date => ({
      date,
      pnl: dailyDataMap[date]
  }));
        ...
        <div className="flex justify-between items-center mb-4 px-2">
            <div className="flex items-center gap-2 bg-black/50 p-1 rounded-lg border border-[#222]">
                <button 
                    ...
                    className={`px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded transition-all ${view === 'equity' ? 'bg-[#222] text-green-400 shadow-sm border border-[#333]' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Equity Curve
                </button>
                <button 
                    onClick={() => setView('daily')}
                    className={`px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded transition-all ${view === 'daily' ? 'bg-[#222] text-blue-400 shadow-sm border border-[#333]' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    Daily P&L
                </button>
            </div>
            ...
            <div className="flex gap-4 text-[10px] text-gray-500 font-mono">
                <span>{view === 'equity' ? 'X: Trade Count' : 'X: Date'}</span>
                <span>Y: USD ($)</span>
            </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {view === 'equity' ? (
                  <AreaChart data={equityData}>
                    ...
                    <YAxis stroke="#4b5563" tick={{fontSize: 10, fontFamily: 'monospace'}} tickFormatter={(val) => `$${val}`} tickLine={false} axisLine={false} />
                    <Tooltip 
                        contentStyle={{ backgroundColor: '#050505', borderColor: '#333', borderRadius: '4px', fontSize: '12px', fontFamily: 'monospace' }}
                        itemStyle={{ color: '#fff' }}
                        formatter={(value) => [`$${value.toFixed(2)}`, 'Equity']}
                        labelFormatter={(label) => `Trade #${label}`}
                    />
                    <Area type="monotone" dataKey="balance" stroke="#22c55e" strokeWidth={2} fillOpacity={1} fill="url(#colorBalance)" animationDuration={1000} />
                  </AreaChart>
              ) : (
                  <BarChart data={dailyData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" vertical={false} />
                    <XAxis dataKey="date" stroke="#4b5563" tick={{fontSize: 10, fontFamily: 'monospace'}} tickLine={false} axisLine={false} />
                    <YAxis stroke="#4b5563" tick={{fontSize: 10, fontFamily: 'monospace'}} tickFormatter={(val) => `$${val}`} tickLine={false} axisLine={false} />
                    <Tooltip 
        ...
        <div className="w-full h-full flex flex-col gap-4">
             <div className="flex items-center justify-between mb-2">
                 <h3 className="text-sm font-bold text-gray-300 uppercase tracking-wider flex items-center gap-2">
                    <PieIcon size={14} className="text-blue-500"/> Distribution
                </h3>
             </div>
             ...
             <div className="flex-1 grid grid-cols-2 gap-2">
                 ...
                 <div className="bg-[#050505] rounded-xl border border-[#222] p-2 relative flex flex-col items-center justify-center">
                    ...
                    <ResponsiveContainer width="100%" height="80%">
                        <PieChart>
                            <Pie data={symbolData} innerRadius={35} outerRadius={50} paddingAngle={5} dataKey="value">
                                {symbolData.map((entry, index) => (
                                    <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} stroke="#050505" />
                                ))}
                            </Pie>
...
const WinRateGauge = ({ percentage }) => {
  const width = 320; const height = 180; const cx = width / 2; const cy = height - 25; 
  const radius = 120; const strokeWidth = 28; const arcLength = Math.PI * radius; 
  const strokeDashoffset = arcLength * (1 - percentage / 100);

  let levelLabel = "Loading";
  let levelColor = "text-gray-500";
  let borderColor = "border-gray-500/30";
  let glowColor = "rgba(107, 114, 128, 0.3)";

  if (percentage < 40) { levelLabel = "Poor"; levelColor = "text-red-500"; borderColor = "border-red-500/30"; glowColor = "rgba(239, 68, 68, 0.3)"; }
  else if (percentage < 55) { levelLabel = "Average"; levelColor = "text-yellow-500"; borderColor = "border-yellow-500/30"; glowColor = "rgba(234, 179, 8, 0.3)"; }
  else if (percentage < 70) { levelLabel = "Good"; levelColor = "text-blue-400"; borderColor = "border-blue-400/30"; glowColor = "rgba(96, 165, 250, 0.3)"; }
  else { levelLabel = "Excellent!"; levelColor = "text-purple-400"; borderColor = "border-purple-400/30"; glowColor = "rgba(192, 132, 252, 0.4)"; }
    ...
    <div className="relative flex flex-col items-center justify-center h-full w-full py-4">
      <div className="relative mt-2" style={{ width: width, height: height }}>
        ...
        <div className="absolute left-0 w-full flex flex-col items-center justify-center" style={{ top: cy - 40 }}>
             ...
             <div className="flex items-center justify-center gap-4 bg-[#121212] px-6 py-2 rounded-full border border-gray-800 shadow-2xl z-10">
                <span className="text-3xl font-bold text-white leading-none tabular-nums -mb-[2px] font-mono">{percentage}%</span>
                <div className="w-[1px] h-5 bg-gray-600/50"></div> 
                <span className="text-[10px] text-gray-500 font-bold tracking-[0.2em] uppercase leading-none mt-[2px]">WIN RATE</span>
             </div>
        </div>
      </div>
    </div>
...
export default function TradingDashboard() {
  ...
  useEffect(() => {
    ...
    setStats({
      ...
      avgTradeSize: total > 0 ? (totalQty / total).toFixed(2) : 0, avgDuration: closedTradesCount > 0 ? formatDuration(totalDurationMs / closedTradesCount) : "0h:00m:00s"
    });
  }, [trades]);
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-200 p-6 md:p-8 font-sans selection:bg-green-500/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8 max-w-[1600px] mx-auto">
      ...
      {/* ROW 3: TABLE */}
      <div className="bg-[#0A0A0A] border border-[#1C1C1C] rounded-2xl p-6 md:p-8 max-w-[1600px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          ...
          <div className="flex flex-wrap gap-3 w-full md:w-auto">
             <div className="relative group">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600 group-hover:text-gray-400 transition-colors" size={14} />
                <input type="text" placeholder="Search Symbol / Ticket..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-[#111] border border-[#222] text-xs text-white pl-9 pr-4 py-2 rounded-lg focus:outline-none focus:border-green-500/50 w-full md:w-48 transition-all" />
             </div>
             <div className="flex bg-[#111] rounded-lg p-1 border border-[#222]">
                {['Open', 'Closed', 'Both'].map((item) => (
                <button key={item} onClick={() => setFilter(item)} className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-md transition-all duration-300 ${filter === item ? 'bg-[#222] text-white border border-gray-600 shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}>{item}</button>
                ))}
            </div>
          </div>
        </div>
              ...
              <tbody className="text-sm">
                {currentItems.length > 0 ? currentItems.map((trade, idx) => (
                  <tr key={idx} className="border-b border-[#1C1C1C]/40 hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="py-4 px-3 text-gray-300 text-[11px] font-mono tabular-nums">{parseFloat(trade.entry).toFixed(2)}</td>
                    <td className="py-4 px-3 text-gray-300 text-[11px] font-mono tabular-nums">{parseFloat(trade.exit).toFixed(2)}</td>
                    <td className="py-4 px-3 text-gray-300 text-[11px] font-mono tabular-nums">{parseFloat(trade.qty).toFixed(2)}</td>
                    <td className={`py-4 px-3 font-bold text-[11px] font-mono tabular-nums ${parseFloat(trade.pnl) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {parseFloat(trade.pnl) >= 0 ? `+${formatCurrency(trade.pnl)}` : formatCurrency(trade.pnl)}
                    </td>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1">
                    {Array.from({length: Math.min(5, totalPages)}, (_, i) => (
                        <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-8 h-8 rounded text-[10px] font-bold ${currentPage === i + 1 ? 'bg-green-500/10 text-green-500 border border-green-500/30' : 'text-gray-500 hover:bg-[#111]'}`}>{i + 1}</button>
                    ))}
                </div>
